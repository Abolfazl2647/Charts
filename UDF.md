...
Response: Library expects to receive a JSON response of the same structure as a result of JS API [onReady() call](https://github.com/Abolfazl2647/Charts/blob/main/JS-Api#onreadycallback).
...
Each property is treated as table column, as described above (see [response-as-a-table](https://github.com/Abolfazl2647/Charts/blob/main/UDF#response-as-a-table-concept)).
The response structure is similar (but **not equal**) to [SymbolInfo](https://github.com/Abolfazl2647/Charts/blob/main/Symbology#symbolinfo-structure) so please read the description to learn about the details of each field.
...
A JSON response of the same structure as [SymbolInfo](https://github.com/Abolfazl2647/Charts/blob/main/Symbology#symbolinfo-structure)
...
- `type`: string. One of the symbol types [supported](https://github.com/Abolfazl2647/Charts/blob/main/JS-Api#symbols_types) by your back-end
- `exchange`: string. One of the exchanges [supported](https://github.com/Abolfazl2647/Charts/blob/main/JS-Api#exchanges) by your back-end
- `limit`: integer. The maximum number of symbols in a response

Example: `GET /search?query=AA&type=stock&exchange=NYSE&limit=15`

A response is expected to be an array of symbol objects as in [respective JS API call](https://github.com/Abolfazl2647/Charts/blob/main/JS-Api#searchsymbolsuserinput-exchange-symboltype-onresultreadycallback)
...
1. Bar time for daily bars should be 00:00 UTC and is expected to be a trading day (not a day when the session starts). Charting Library aligns the time according to the [Session](https://github.com/Abolfazl2647/Charts/blob/main/Symbology#session) from SymbolInfo.
...
This object is similar to [respective response](https://github.com/Abolfazl2647/Charts/blob/main/JS-Api#getmarkssymbolinfo-from-to-ondatacallback-resolution) in JS API, but each property is treated as a table column, as described above.
